...
public class PatchClass {
    ...
    public static void process(String buildDir, String lib) {
        ...
        CtClass c = classes.getCtClass("vidic.hotupdataapp.BugClass")
        if (c.isFrozen()) {
            c.defrost()
        }
        println("====添加构造方法====")
        def constructor = c.getConstructors()[0];
        constructor.insertBefore("System.out.println(vidic.hotupdataapp.AntilazyLoad.class);")
        c.writeFile(buildDir)


        CtClass c1 = classes.getCtClass("vidic.hotupdataapp.LoadBugClass")
        if (c1.isFrozen()) {
            c1.defrost()
        }
        println("====添加构造方法====")
        def constructor1 = c1.getConstructors()[0];
        constructor1.insertBefore("System.out.println(vidic.hotupdataapp.AntilazyLoad.class);")
        c1.writeFile(buildDir)


        CtClass c2 = classes.getCtClass("vidic.hotupdataapp.ListActivity")
        if (c2.isFrozen()) {
            c2.defrost()
        }
        println("====添加构造方法====")
        def constructor2 = c2.getConstructors()[0];
        constructor2.insertBefore("System.out.println(vidic.hotupdataapp.AntilazyLoad.class);")
